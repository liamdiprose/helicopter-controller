...
Button *button_init(uint32_t gpio_periph, uint32_t gpio_base, uint32_t gpio_pin,
		    ButtonPullDirection pull_direction)
{
	...
	case PULL_DOWN:
		{
			GPIOPadConfigSet(gpio_base, gpio_pin, GPIO_STRENGTH_2MA,
					 GPIO_PIN_TYPE_STD_WPU);
			new_button = (Button) {
			gpio_base, gpio_pin, pull_direction, 0,
	...
	case PULL_UP:
		{
			...
			new_button = (Button) {
			gpio_base, gpio_pin, pull_direction,
				    BUTTON_COUNT_START,
...
void button_check_routine(void)
{
	...
	for (i = 0; i < BUTTON_NUM; i++) {
		...
		current_pin_state =
		    GPIOPinRead(current_button->gpio_base,
				current_button->gpio_pin) > 0;
		...
		} else if (current_button->count == BUTTON_COUNT_START
			   && current_button->state == BUTTON_STATE_RELEASED) {
			current_button->state = BUTTON_STATE_PRESSED;
			current_button->updated = 1;
		}
